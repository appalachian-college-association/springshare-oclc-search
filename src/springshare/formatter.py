...
def format_results(results: Dict, library_symbol: str = None) -> Dict:
    ...
    """
    try:
        logger.info(f"Formatting results for library symbol: {library_symbol}")
        config = Config()
        ...
        logger.info(f"Using discovery site: {site}")
        ...
        for item in items:
            ...
            formatted_item = {
                "title": title,  # Original title preserved here
                "ocn": oclc_number,
                "url": format_worldcat_url(title, site, config),  # Pass config for URL processing
